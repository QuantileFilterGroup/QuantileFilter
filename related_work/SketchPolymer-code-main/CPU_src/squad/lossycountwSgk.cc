...
LCU_type * LCU_Init(float fPhi, double qeps, long long samples_num)
{
	...
	for (i=0;i<k;i++) 
	{
		...
		result->items[i].qsketch = new GK<long double>(result->qsketch_eps);
...
void LCU_InsertIntoHashtable(LCU_type *lcu, LCUITEM *newi, int i, uint64_t newitem){
...
LCUITEM * LCU_GetNewCounter(LCU_type * lcu) {
	...
	delete (newi->qsketch);
    newi->qsketch = new GK<long double>(lcu->qsketch_eps);
	newi->timestamp = ts;
	return (newi);
}
...
void LCU_addToSample(LCU_type * lcu, uint64_t newitem, long double latency) {
	++lcu->nb_processed;
	sampleEntry se;
	se.flow = newitem;
	se.latency = latency;
...
void LCU_UpdateLatency(LCU_type * lcu, uint64_t newitem, long double latency) {
	int h;
	LCUITEM *il;
...
void LCU_Update(LCU_type * lcu, uint64_t newitem) {
	int h;
	LCUITEM *il;
...
long double LCU_QuantileQuery(LCU_type * lcu, uint64_t newitem, double rank) {
	int h;
	LCUITEM *il;
	...
	il->qsketch->finalize();
	long double val = il->qsketch->query_for_value(rank);
  	return val;
}

long double LCU_QuantileQuerywGsamples(LCU_type * lcu, uint64_t newitem, double rank) {
	int h;
	LCUITEM *il;
	std::vector<long double> sample_buff;
	GK<long double>* tmpsketch = new GK<long double>(lcu->qsketch_eps);
	...
	tmpsketch->finalize();
	long double val = tmpsketch->query_for_value(rank);
  	return val;
}
