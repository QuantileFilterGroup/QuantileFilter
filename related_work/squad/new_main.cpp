...
int main (int argc, char **argv) {
	...
	for (time = 0; ; ++time) {
		...
		*/
		...
		// long double res = LCU_QuantileQuery (squad, quin_id, PERCENTAGE - (double) DELTA / SKETCH_THRESHOLD);
		if (res > THRESHOLD) {
			alerted.insert (s);
			++count_alert;
			fprintf (file, "0x");
