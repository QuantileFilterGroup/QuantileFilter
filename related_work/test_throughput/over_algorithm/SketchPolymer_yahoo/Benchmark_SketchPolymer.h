    ...
    void Run(uint32_t memory) 
    {
        ...
        for (int i = 0; i < running_length; ++i) 
        {
            ...
            if (id_map[dataset[i].id] > 1) 
            {
                ...
                begin=clock();
                sketchpolymer->insert(dataset[i].id, 0, dataset[i].timestamp);
                finish=clock();
            ...
            if (id_map[dataset[i].id] > 1) 
            {
                ...
                //sketchpolymer->insert(dataset[i].id, last_time[dataset[i].id], i);
            }
            #endif
        }
        ...
        for (auto i : id_map) 
        {
            ...
            uint64_t predict = sketchpolymer->query(i.first, query_w);
    ...
    void Run(uint32_t memory) 
    {
        ...
        for (int i = 0; i < running_length; ++i) 
        {
            ...
            if (id_map[dataset[i].id] > 1 && dataset[i].timestamp > last_time[dataset[i].id]) 
            {
                ...
                begin=clock();
                sketchpolymer->insert(dataset[i].id, last_time[dataset[i].id], dataset[i].timestamp);
                finish=clock();
