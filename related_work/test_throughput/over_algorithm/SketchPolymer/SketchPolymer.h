...
public:
	...
	Stage4(uint32_t memory) {
		size = memory / sizeof(uint32_t) / d4;
		bf = new uint32_t* [d4];
		for (int i = 0; i < d4; ++i) {
...
public:
	...
	Stage3Tower(uint32_t memory) {
		mem = memory;
		uint32_t cell_num = memory / sizeof(uint32_t) / 3;
	...
	void print_status()
	{
		...
		uint32_t cell_num = mem / sizeof(uint32_t) / 3;
