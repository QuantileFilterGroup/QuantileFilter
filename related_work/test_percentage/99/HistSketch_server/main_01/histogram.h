...
public:
	...
	static size_t get_memory_usage() {
		 ...
		 * 		The designers of HistSketch didn't seem to calculate their hash parameters..
		 * 		So I helped them :)
		*/
		size_t hash_parameters = sizeof (int64_t) * 3 * HashNum;
		return hash_parameters +
			sizeof(T) * bucket_num + bucket_num / 8 + (int)(log(BUCKET_NUM) / log(2)) * bucket_num / 8;	// counter + flag + idx
	}
};
