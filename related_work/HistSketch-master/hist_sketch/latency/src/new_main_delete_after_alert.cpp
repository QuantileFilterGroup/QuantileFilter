...
std::map <std::vector <uint8_t>, std::pair<int32_t, int32_t>> last_alerted;

int main() {
	assert (PERCENTAGE >= 0.5);
	assert (PERCENTAGE < 1 - 1e-3);
	const char input_name [] = "../../../../../130000_delay.dat";
	const char output_name [] = "hist_sketch.out";
	...
	for (time = 0; ; ++time) {
		...
		quin.vectorize (key);
		uint8_t bid = time > THRESHOLD;
		hist.insert ((Key_t) (&(*key.begin ())), bid);
		/**
		 * Problem 1: Original Hist Sketch doesn't support deletion.
		 * 			However, since it returns the number of items
		 * 			in both sides, we could record the pair every
		 * 			time there's an alerting. Then we would use the
		 * 			delta to determine whether to alert or not.
		*/
		auto it = last_alerted.find (key);
		std::pair <int32_t, int32_t> res (0, 0);
		if (it != last_alerted.end ())
			res = it->second;
		Hist_t result = hist.histogramQuery ((Key_t) (&(*key.begin ())));
		int32_t small_equ = (int32_t) result.at (0) - res.first;
		int32_t large = (int32_t) result.at (1) - res.second;
		/** Abnormal. Alert and add it to ALERTED. */
		if (small_equ <= (int32_t) ((small_equ + large) * PERCENTAGE - DELTA)) {
			last_alerted [key] = std::make_pair ((int32_t) result.at (0), (int32_t) result.at (1));
			++count_alert;
			fprintf (file, "0x");
			for (auto &v : key)
				fprintf (file, "%02X", v);
