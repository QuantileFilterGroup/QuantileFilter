...
#define SKETCH_WIDTH (3 * (1 << 14) / 5)
#define SKETCH_NUMBER 5
	...
	public:
		...
		*/
		int16_t calculate (const uint32_t &fp) {
			...
			for (int i = 0; i < SKETCH_NUMBER; ++i) {
				ret += sketch[i][lightFPs[i] % SKETCH_WIDTH];
			}
			return (int8_t) (ret / SKETCH_NUMBER);
		}
		...
		*/
		void insertLight (uint32_t fp, int16_t value) {
			...
			for (int i = 0; i < SKETCH_NUMBER; ++i) {
				...
				sketch [i][j] = protectedPlus_8 (sketch [i][j], (int8_t) value);
			}
			return;
		}
		...
		*/
		void insert (const Quintet &quin, uint32_t current_time) {
			...
			for (int i = 0; i < SKETCH_NUMBER; ++i) {
				int j = LightFPs [i] % SKETCH_WIDTH;
				sketch [i][j] = protectedPlus_8 (sketch [i][j], -1);
			}
