	...
	public:
		...
		*/
		std::pair <uint32_t, int> getPair (const Quintet &quin) {
			...
			uint32_t long_fp = (uint32_t) (murmur3_32 (quin, seeds [0]) % TOTAL_FP_MASK);
		...
		*/
		inline uint32_t getBlockIndex (const uint32_t &fp) {
			...
			return (uint32_t) ((fp % TOTAL_FP_MASK) >> 16);
		}
		...
		*/
		...
		uint32_t reviveFullFP (uint32_t index, uint16_t fp) {
			...
			return (uint32_t) (((index << 16) | fp) % TOTAL_FP_MASK);
		}
...
int main (int argc, char **argv) {
	...
	const char output_name [] = "optimized_variant_block_length.out";
