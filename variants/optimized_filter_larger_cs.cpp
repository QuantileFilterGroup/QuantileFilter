...
class Sketch {
	private:
		uint16_t count [FILTER_NUMBER][FILTER_WIDTH];
	...
	public:
		...
		*/
		...
		void init (int argc, char **argv, const char *file_name) {
			...
			for (int i = 0; i < SKETCH_NUMBER + FILTER_NUMBER + 2; ++i) {
				seeds.push_back (rnd ());
			}
		...
		*/
		...
		/**
		 * Calculate finger prints for the filter, using its
		 * full finger print.
		*/
		std::vector <uint32_t> getFilterFPs (uint32_t fp) {
			std::vector <uint32_t> ret;
			for (int i = SKETCH_NUMBER + 2; i < SKETCH_NUMBER + FILTER_NUMBER + 2; ++i) {
				ret.push_back (murmur3_32_single (fp, seeds [i]));
			}
			return ret;
		}
		...
		*/
		void insert (const Quintet &quin, uint32_t current_time) {
			...
			auto filterFPs = getFilterFPs (pair.first);
			uint16_t min_count = 0xFFFFu;
			for (int i = 0; i < FILTER_NUMBER; ++i) {
				int j = filterFPs [i] % FILTER_WIDTH;
				min_count = std::min (min_count, count [i][j]);
			}
			/**
			 * The stream is too small to be an abnormal stream.
			*/
			if (min_count < DELTA) {
				for (int i = 0; i < FILTER_NUMBER; ++i) {
					int j = filterFPs [i] % FILTER_WIDTH;
...
int main (int argc, char **argv) {
	...
	const char output_name [] = "optimized_filter_larger_cs.out";
