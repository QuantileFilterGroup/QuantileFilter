...
class Sketch {
	private:
		uint16_t count [DUAL_NUMBER][DUAL_WIDTH];
	...
	public:
		...
		*/
		...
		void init (int argc, char **argv, const char *file_name) {
			...
			for (int i = 0; i < SKETCH_NUMBER + DUAL_NUMBER + 2; ++i) {
				seeds.push_back (rnd ());
			}
		...
		*/
		...
		/**
		 * Calculate finger prints for the dual part, using its
		 * full finger print.
		*/
		std::vector <uint32_t> getDualFPs (uint32_t fp) {
			std::vector <uint32_t> ret;
			for (int i = SKETCH_NUMBER + 2; i < SKETCH_NUMBER + DUAL_NUMBER + 2; ++i) {
				ret.push_back (murmur3_32_single (fp, seeds [i]));
			}
			return ret;
		}
		...
		*/
		int16_t calculate (const uint32_t &fp) {
			...
			for (int i = 0; i < SKETCH_NUMBER; ++i) {
				numbers.push_back ((int32_t) sketch[i][lightFPs[i] % SKETCH_WIDTH]);
			}
		...
		*/
		void insertLight (uint32_t fp, int16_t value) {
			...
			assert (lightFPs.size () == SKETCH_NUMBER + 1);
			int16_t sign = (lightFPs [SKETCH_NUMBER] & 1) ? -1 : 1;
			for (int i = 0; i < SKETCH_NUMBER; ++i) {
				int j = lightFPs [i] % SKETCH_WIDTH;
				sketch [i][j] = protectedPlus (sketch [i][j], (int16_t) (sign * value));
			}
			return;
		}
		...
		*/
		void insert (const Quintet &quin, uint32_t current_time) {
			...
			auto dualFPs = getDualFPs (pair.first);
			for (int i = 0; i < DUAL_NUMBER; ++i) {
				int j = dualFPs [i] % DUAL_WIDTH;
				if (count [i][j] != 0xFFFFu)
					++count [i][j];
				min_count = std::min (min_count, count [i][j]);
			}
			...
			if (pair.second > THRESHOLD) {
				...
				uint16_t min_count_tmp = 0xFFFFu;
				auto dualFPs_tmp = getDualFPs
					(reviveFullFP (argmin.first, heavy_fp [argmin.first][argmin.second]));
				for (int i = 0; i < DUAL_NUMBER; ++i) {
					int j = dualFPs_tmp [i] % DUAL_WIDTH;
					min_count_tmp = std::min (min_count_tmp, count [i][j]);
				}

				/** Not replacing. Just let new value go back. */
				if ((int32_t) min_count_tmp + tmp_val >= (int32_t) min_count + sign * new_val) {
...
int main (int argc, char **argv) {
	...
	const char output_name [] = "optimized_dual_larger_cs.out";
