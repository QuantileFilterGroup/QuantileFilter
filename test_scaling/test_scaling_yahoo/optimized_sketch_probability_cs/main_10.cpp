...
#define BLOCK_NUMBER_BITS 14
#define BLOCK_NUMBER (1 << BLOCK_NUMBER_BITS << 1)
#define BLOCK_LENGTH 6
#define FP_MASK ((1 << 16) - 1)
#define TOTAL_FP_MASK ((1ll << (16 + BLOCK_NUMBER_BITS) << 1) - 1)
		...
		*/
		void insert (const Quintet &quin, uint32_t current_time) {
			...
			if (pair.second > THRESHOLD) {
				 ...
				 * the heavy part if it's lucky.
				 */
				auto argmin = getHeavyMin (pair.first);
				int16_t tmp_val = heavy_value [argmin.first][argmin.second];
				bool flag = false;
				if (sign * new_val <= 0 || tmp_val <= 0) {
					/**
					 * If either of the two is no large than 0,
					 * just compare them to determine to replace or not.
					 */
					if (tmp_val >= sign * new_val)
						flag = true;
				}
				else {
					int sum = (int) (sign * new_val) + (int) tmp_val;
					double p = (double) tmp_val / sum;
					if (rnd () <= p * rnd.max ())
						flag = true;
				}
