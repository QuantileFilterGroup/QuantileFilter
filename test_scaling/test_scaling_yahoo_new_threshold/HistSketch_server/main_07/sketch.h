...
public:
	...
	void insert(Key_t k, uint8_t bid, T v = 1) {
		new_data_t swap_key;
		...
		switch (ret) {
			...
			{
				new_data_t tmp = new_data_t(k, bid);
	...
	T pointQuery(Key_t k, uint8_t bid) {
		...
		five_tuple tmpKey(k);
		if (evictHistResult.find(tmpKey) != evictHistResult.end() && evictHistResult[tmpKey].find(bid) != evictHistResult[tmpKey].end()) {
	...
	uint32_t get_cardinality() {
		...
		for (int i = 0; i < slot_num; ++i) {
			...
			for (int j = 0; j < BUCKET_NUM; ++j) {
				...
				if (evictHistResult.find(cur_five_tuple) != evictHistResult.end() && evictHistResult[cur_five_tuple].find(j) != evictHistResult[cur_five_tuple].end())
	...
	size_t get_memory_usage() {
		std::cout << "Hash table:" << table.get_memory_usage() << std::endl;
		std::cout << "CM Sketch:" << lightHist.get_memory_usage() << std::endl;
		std::cout << "Bloom Filter:" << bfHist.get_memory_usage() << std::endl;
		/**
		 * Tester of QuantileFilter:
		 * 		The designers of HistSketch didn't seem to calculate their server memory usage...
		 * 		So I helped them :)
		*/
		size_t res = 0;
		for (auto &v : evictHistResult)
			res += v.second.size ();
		std::cout << "flowKeysHist" << flowKeysHist.size () << std::endl;
		std::cout << "evictHistResult" << evictHistResult.size () << ' ' << res << std::endl;
		std::cout << "cmHistResult" << cmHistResult.size () << std::endl;
		return flowKeysHist.size () * (CHARKEY_LEN + sizeof (uint8_t)) + sizeof (flowKeysHist) +
			evictHistResult.size () * (CHARKEY_LEN + sizeof (std::map<uint8_t, uint32_t>)) +
			sizeof (evictHistResult) + res * (sizeof (uint8_t) + sizeof (uint32_t)) +
			cmHistResult.size () * (CHARKEY_LEN + sizeof (std::array<T, BUCKET_NUM>)) +
			sizeof (cmHistResult) +
			table.get_memory_usage() + lightHist.get_memory_usage() + bfHist.get_memory_usage();
	}
};
